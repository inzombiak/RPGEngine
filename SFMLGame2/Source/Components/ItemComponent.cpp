...
void Item::Apply(StrongEntityPtr target)
{
	...
	*/
	...
	if (iter != m_itemComponents.end())
	{
		...
		if (CheckConvertAndCastPtr<IComponent, VitalsComponent>(target->GetComponent(IComponent::GetIDFromName(VitalsComponent::COMPONENT_NAME)), targetVitals))
		{
	...
	else
	{
		...
		if (CheckConvertAndCastPtr<IComponent, EquipmentComponent>(target->GetComponent(IComponent::GetIDFromName(EquipmentComponent::COMPONENT_NAME)), equipComp))
		{
			Item oldItem;
			if (equipComp->Equip(equipItem->GetSlot(), (*this), oldItem))
			{
				std::shared_ptr<InventoryComponent> invComp;
				if (CheckConvertAndCastPtr<IComponent, InventoryComponent>(target->GetComponent(IComponent::GetIDFromName(InventoryComponent::COMPONENT_NAME)), invComp))
				{
