...
void UIManager::PostInit()
{
	...
	if (player)
	{
		...
		CheckConvertAndCastPtr(player->GetComponent(IComponent::GetIDFromName(InventoryComponent::COMPONENT_NAME)), m_playerInventory);

		//Get players vitals
		std::shared_ptr<VitalsComponent> playerVitals;
		if (CheckConvertAndCastPtr(player->GetComponent(IComponent::GetIDFromName(VitalsComponent::COMPONENT_NAME)), playerVitals))
		{
		...
		if (CheckConvertAndCastPtr(player->GetComponent(IComponent::GetIDFromName(StatComponent::COMPONENT_NAME)), playerStats))
			m_characterWindow->SetStats(playerStats);

		//Get player equipment
		CheckConvertAndCastPtr<IComponent, EquipmentComponent>(player->GetComponent(IComponent::GetIDFromName(EquipmentComponent::COMPONENT_NAME)), m_playerEquipment);
	}
}
...
void UIManager::UpdatePlayerVitals()
{
	...
	if (player)
	{
		WeakComponentPtr wVitalsPtr = player->GetComponent(IComponent::GetIDFromName(VitalsComponent::COMPONENT_NAME));
